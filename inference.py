...
async def run_inference():
    ...
    try:
        ...
        print("[DEBUG] Waiting for container to start and become healthy...", flush=True)
        backend_ready = False
        for _ in range(15):
            try:
                with httpx.Client() as htest:
                    res = htest.get(f"{env_url}/health")
        ...
        if not backend_ready:
            print("[DEBUG] Container failed to start or healthcheck timed out.", flush=True)
            return
            
        print("[DEBUG] Container is ready. Starting inference...", flush=True)
